...
// =====================================================================
// LazyImporter v2 — Hardened
//
// Melhorias sobre o original:
//   1. unsafe_module_enumerator agora tem terminação (circular list detection)
//      — evita loop infinito se export/módulo não for encontrado
//   2. Hash com Murmur3 finalizer pós-FNV-1a — melhor avalanche,
//      dificulta bruteforce reverso mesmo com __TIME__/__DATE__ conhecido
//   3. Validação de PE magic (MZ + PE) antes de parsear headers
//      — não crasha em entry corrompida ou módulo sendo unloaded
//   4. exports_directory valida VirtualAddress != 0
//      — não crasha em DLLs sem export table
//   5. in() SEMPRE faz bounds check — o loop infinito sem IsSafe foi eliminado
//   6. Case-insensitive por default no module matching
//      — "opengl32.dll" == "OpenGL32.DLL" sem precisar de .safe()
// =====================================================================
...
FrameWork::LazyImporter::li::detail::khash_impl( __TIME__ __DATE__, 2166136261 ))
    ...
    namespace LazyImporter
    {
        namespace li
        {
            namespace detail
            {
                ...
                // ─────────────────────────────────────────────
                // [MELHORIA 2] Hash com Murmur3 finalizer
                // FNV-1a por char + finalizer no resultado
                // Melhor avalanche — 1 bit de diferença no input
                // muda ~50% dos bits do hash final
                // ─────────────────────────────────────────────
                ...
                // Murmur3 32-bit finalizer — constexpr
                LAZY_IMPORTER_FORCEINLINE constexpr unsigned murmur3_finalize(unsigned h) noexcept
                {
                    h ^= h >> 16;
                    h *= 0x85ebca6b;
                    h ^= h >> 13;
                ...
                {
                    return (*str ? khash_impl(str + 1, hash_single(value, *str)) : murmur3_finalize(value));
                }
                ...
                // Runtime hash — também com finalizer
                    ...
                    const char* str, unsigned offset) noexcept
                {
                    ...
                    // Finalizer em ambos
                    res.module_hash = murmur3_finalize(res.module_hash);
                    res.function_hash = murmur3_finalize(res.function_hash);

                    return res;
                }

                // ─────────────────────────────────────────────
                // [MELHORIA 3] Validação de PE magic
                // Checa MZ (0x5A4D) e PE\0\0 (0x00004550)
                // ─────────────────────────────────────────────

                LAZY_IMPORTER_FORCEINLINE bool is_valid_pe(const char* base) noexcept
                {
                    if (!base) return false;
                    auto dos = reinterpret_cast<const win::IMAGE_DOS_HEADER*>(base);
                    if (dos->e_magic != 0x5A4D) return false;
                    auto nt = reinterpret_cast<const win::IMAGE_NT_HEADERS*>(base + dos->e_lfanew);
                ...
                LAZY_IMPORTER_FORCEINLINE const win::IMAGE_EXPORT_DIRECTORY* image_export_dir(
                    const char* base) noexcept
                {
                    // [MELHORIA 4] Valida VirtualAddress antes de deref
                    const auto& data_dir = nt_headers(base)->OptionalHeader.DataDirectory[0];
                    if (!data_dir.VirtualAddress)
                        return nullptr;
                    return reinterpret_cast<const win::IMAGE_EXPORT_DIRECTORY*>(
                        base + data_dir.VirtualAddress);
                }
                ...
                struct exports_directory {
                    ...
                    unsigned long                      _image_size; // for EAT hook validation
                    const char* _base;
                ...
                public:
                    ...
                    LAZY_IMPORTER_FORCEINLINE
                        exports_directory(const char* base) noexcept : _base(base), _ied(nullptr), _ied_size(0), _image_size(0)
                    {
                        // [MELHORIA 3+4] Valida PE + export dir antes de tudo
                        if (!is_valid_pe(base))
                            return;

                        const auto* nt = nt_headers(base);
                        _image_size = nt->OptionalHeader.SizeOfImage;

                        const auto& ied_data_dir = nt->OptionalHeader.DataDirectory[0];
                        if (!ied_data_dir.VirtualAddress)
                            return;
                    ...
                    {
                        return _ied != nullptr && reinterpret_cast<const char*>(_ied) != _base;
                    }

                    LAZY_IMPORTER_FORCEINLINE size_type size() const noexcept
                    {
                        return _ied ? _ied->NumberOfNames : 0;
                    }
                    ...
                    LAZY_IMPORTER_FORCEINLINE const char* address(size_type index) const noexcept
                    {
                        ...
                        const auto rva = rva_table[ord_table[index]];

                        // EAT hook validation: RVA must fall within module image.
                        // If attacker patched AddressOfFunctions to point outside
                        // the module, the RVA will exceed SizeOfImage — reject it.
                        if (_image_size && rva >= _image_size)
                            return nullptr;
                ...
                // ─────────────────────────────────────────────
                // [MELHORIA 1] unsafe_module_enumerator com terminação
                // Detecta volta ao head da circular list + max 256 módulos
                // ─────────────────────────────────────────────
                ...
                struct unsafe_module_enumerator {
                    using value_type = const detail::win::LDR_DATA_TABLE_ENTRY_T*;
                    value_type value;
                    value_type head;            // [NOVO] salva head pra detectar ciclo
                    unsigned   iterations;      // [NOVO] safety counter

                    LAZY_IMPORTER_FORCEINLINE unsafe_module_enumerator() noexcept
                        : value(ldr_data_entry()), head(value), iterations(0)
                    {
                    }
                    ...
                    LAZY_IMPORTER_FORCEINLINE bool next() noexcept
                    {
                        value = value->load_order_next();
                        ++iterations;
                        // Termina se: voltou ao head OU DllBase null OU >256 módulos
                        // (nenhum processo real tem 256 DLLs na InLoadOrderModuleList)
                        if (value == head || !value->DllBase || iterations > 256)
                            return false;
                        return true;
                    }
                };
                ...
                struct lazy_function : lazy_base<lazy_function<OHP, T>, T> {
                    ...
                    template<class... Args>
                    LAZY_IMPORTER_FORCEINLINE auto operator()(Args&&... args) const
                    {
                        return base_type::template cached<T>()(LAZY_IMPORTER_CPP_FORWARD(Args, args)...);
                    }
                    ...
                    LAZY_IMPORTER_FORCEINLINE static F get() noexcept
                    {
                        ...
                        do {
                            // [MELHORIA 3] PE validation antes de parsear exports
                            if (!is_valid_pe(e.value->DllBase))
                                continue;
                            ...
                            if (exports) {
                                ...
                                while (export_index--)
                                    if (hash(exports.name(export_index), get_offset(OHP)) == get_hash(OHP)) {
                                        ...
                                        if (addr) // skip EAT-hooked entries (RVA outside module)
                                            return (F)(addr);
                                    }
                            }
                        } while (e.next());
                    ...
                    // ─────────────────────────────────────────────
                    // [MELHORIA 5] in() SEMPRE faz bounds check
                    // O loop infinito do original foi eliminado
                    // IsSafe mantido pra backward compat mas agora
                    // ambos os paths fazem bounds check
                    // ─────────────────────────────────────────────
                    ...
                    LAZY_IMPORTER_FORCEINLINE static F in(Module m) noexcept
                    {
                        ...
                        // Valida PE antes de parsear
                        if (!is_valid_pe((const char*)(m)))
                            return {};
                        ...
                        // SEMPRE bounds-checked — sem loop infinito
                        const auto count = exports.size();
                        for (unsigned long i = 0; i < count; ++i) {
                            if (hash(exports.name(i), get_offset(OHP)) == get_hash(OHP)) {
                                const auto addr = exports.address(i);
                                if (addr) // skip EAT-hooked entries
                                    return (F)(addr);
                            }
                        }
                        return {};
